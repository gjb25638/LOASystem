...
function haveSignersOfRecordAllSigned(record) {
    return record.signers.every(signer => record.signings.some(signing => signing.id === signer.id))
}
...
module.exports = {
    ...
    predicate: {
        ...
        isRecordAllSignedPass: (record) => {
            ...
            const allSignersOfTheRecordHaveSigned = haveSignersOfRecordAllSigned(record)
            return record.signers.length > 0 ?
                allSignersOfTheRecordHaveSigned && record.signings.every(signing => signing.pass) :
                true
        },
        ...
        isLeaveTypeNoMoreLeaves: (leaveType) => {
            return leaveType.totals.days === leaveType.consumes.days &&

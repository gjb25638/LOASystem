...
function produce({ loginuser, employees, year, month, groupby, sdehsra }) {
  return {
    ...
    report: month
      ? produceMonthly({ loginuser, employees, year, month, groupby, sdehsra })
      : produceAnnual({ loginuser, employees, year, groupby })
  };
}
  ...
  return employees
    ...
    .map(employee => {
      ...
      if (groupby) {
        ...
        const leaveTypeGroups = groupByLeaveType(records).map(
          leaveTypeGroup => {
            leaveTypeGroup.totals = u.calculateTotals(leaveTypeGroup.list);
            return leaveTypeGroup;
          }
        );
        const annualLeaves = employee.records.filter(
          record =>
  ...
  return employees
    ...
    .map(employee => {
      ...
      if (groupby) {
        ...
        bascInfo.records = records.map(record =>
          Object.assign({}, record, {
            dates: filterMatchedDates(record.dates, { month: month })
          })
        );
        return bascInfo;
      }
    ...
    .map((dateGroup, index) => {
      const result = {
        list: dateGroup.map(leaveTypeGroup =>
          u.renew(leaveTypeGroup, {
            totals: u.calculateTotals(leaveTypeGroup.list)
          })
        )
      };
...
function transformToAnnualInfo(annualLTRecords, dateRanges) {
  return dateRanges.map(x => ({
    ...
    totalDays: x.days,
    totals: u.calculateTotals(
      filterBetweenDates(annualLTRecords, x.start, x.end)
    )
  }));
}
...
function filterWithin(records, year, month) {
  ...
  records.forEach(record => {
    ...
    if (anyDateOfRecordWithin) {
      const dates = filterMatchedDates(
        record.dates,
        month ? { month } : { year }
      );
      result.push(
        u.renew(record, {
          dates,
          totals: { days: dates.length, halfHours: record.totals.halfHours }
        })
      );
    }
  });
...
function groupBy(records, month) {
  ...
  records.forEach(record => {
    record.dates.map(u.toDateInfo).forEach(dateInfo => {
...
function filterBetweenDates(records, start, end) {
  ...
  records.forEach(record => {
    const anyDatesOfRecordBetweenDates = record.dates
      .map(u.toDateInfo)
      .some(
        dateInfo => dateInfo && dateInfo.date < end && dateInfo.date >= start
      );
...
function isFutureRecord(record, year, month) {

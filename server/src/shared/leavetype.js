...
function refresh(employee, cb = undefined) {
  ...
  const enabledLeaveTypes = employee.activatedLeaveTypes.filter((lt) => lt.enabled)
  const refreshedLeaveTypes = refreshBasicLeaveTypes(leaveTypesGroups, enabledLeaveTypes)

  const annualLT = findLeaveType(enabledLeaveTypes, "annual")
  const annualPreRequestLT = findLeaveType(enabledLeaveTypes, "annualPreRequest")
  const arrivedDate = new Date(employee.arrivedDate)
  if (annualLT && annualPreRequestLT && isAnnualLTRefreshable(annualLT.deadline, arrivedDate)) {
    refreshAnnualLT(arrivedDate, annualLT, annualPreRequestLT, employee.records)
...
function refreshAnnualLT(arrivedDate, annualLT, annualPreRequestLT, records, specificDate) {
  const deadline = getAnnualLTDeadline(arrivedDate, specificDate)
  const { totalsDaysThisYear, totalsDaysNextYear } = getAnnualLTTotalDays(arrivedDate, specificDate)
...
function refreshAnnualPreRequestLT(deadline, annualPreRequestLT, totalsDays) {
  annualPreRequestLT.deadline = new Date(deadline.getFullYear() + 1, deadline.getMonth(), deadline.getDate())
...
function getLTDeadLine(period, specificToday = undefined) {
  ...
  if (period === "year") {
    ...
    return currentMonth < 11 ? new Date(currentYear, currentMonth + 1, 0) : new Date(currentYear + 1, 0, 0)
  }
}
...
function getAnnualLTDeadline(arrivedDate, specificToday = undefined) {
  ...
  if (monthAmount >= 6) {
    ...
    return deadlineCandidates[0] >= today ? deadlineCandidates[0] : deadlineCandidates[1]
  }
}

function isAnnualLTRefreshable(deadline, arrivedDate, specificToday = undefined) {
  return !deadline || deadline < getAnnualLTDeadline(arrivedDate, specificToday)
}
...
function getAnnualLTTotalDays(arrivedDate, specificToday = undefined) {
  ...
  return {
    totalsDaysThisYear: monthAmount >= 6 ? getAnnualLTTotalDaysBySeniority(seniority) : 0,
    totalsDaysNextYear: monthAmount >= 6 ? getAnnualLTTotalDaysBySeniority(seniority + 1) : getAnnualLTTotalDaysBySeniority(seniority)
  }
}

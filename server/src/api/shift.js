...
const holidayConfig = path.join(configDir, "holiday.json")
        ...
        if (err) {
            ...
            Employee.find(c.conditions.all(), null, (err, employees) => {
                if (err) {
                    ...
                    res.send(produce(loginuser, employees, parseInt(req.params.year), parseInt(req.params.month)))
                }
            })
        } else {
                    ...
                    if (
                        ...
                        employee.save((err) => {
                            if (err) {
                                ...
                                res.send({ success: true, message: "shift updated successfully" })
                            }
                        })
                    } else {
                        res.send({ success: false, message: "unable to update expired shift" })
                    }
                }
            })
        } else {
        ...
        if (err) {
            ...
            Employee.findById(req.params.id, null, (err, employee) => {
                if (err) {
                    ...
                    const shift = employee.shifts.find(shift => shift._id.toString() === req.body.shiftId)
                    const shiftDateInfo = u.toDateInfo(shift.date)
                    const currentDateInfo = u.toDateInfo(new Date())
                    if (shiftDateInfo.month >= currentDateInfo.month &&
                        shiftDateInfo.year >= currentDateInfo.year) {
                        employee.shifts = employee.shifts.filter(shift => shift._id.toString() !== req.body.shiftId)
                        employee.save((err) => {
                            if (err) {
                                res.send({ success: false, message: err })
                            } else {
                                res.send({ success: true, message: "shift deleted successfully" })
                            }
                        })
                    } else {
                        res.send({ success: false, message: "unable to update expired shift" })
...
function updatePeriod(req, res) {
    validate.manager(req.params.loginuser, req.params.token, (err, pass) => {
        if (err) {
            res.send({ success: false, message: err })
        } else if (pass) {
            fs.writeFileSync(periodConfig, JSON.stringify(req.body.config), { encoding: 'utf8' })
            res.send({ success: true, message: "shift config updated successfully" })
        }
        else {
            res.send({ success: false, message: "token validation failed" })
        }
...
function getHoliday(req, res) {
    Employee.findOne(c.conditions.validLoginuser(req.params.loginuser, req.params.token), (err, loginuser) => {
        if (err) {
            ...
            const content = fs.readFileSync(holidayConfig, { encoding: 'utf8' })
            res.send(content)
        } else {
            res.send({ success: false, message: "login user is not found" })
        }
...
function updateHoliday(req, res) {
    validate.manager(req.params.loginuser, req.params.token, (err, pass) => {
        if (err) {
            ...
            fs.writeFileSync(holidayConfig, JSON.stringify(req.body.config), { encoding: 'utf8' })
            res.send({ success: true, message: "shift config updated successfully" })
        }
        else {
            res.send({ success: false, message: "token validation failed" })
...
function produce(loginuser, employees, year, month) {
    return {
        ...
        employees: employees
            .filter(employee => c.predicate.isEmployeeUnderLoginuserControl(loginuser, employee, true, true))
            .map(employee => {
                return {
                    _id: employee._id,

          ...
          updateEmployee(employee, req.body).save(err => {
            if (err) {
              ...
              if (lightweight) {
                ...
                Employee.find(
                  c.conditions.underControl(employee.username),
                  (err, employees) => {
                    if (err) {
                      res.send({ success: false, message: err });
                    } else if (employees && employees.length > 0) {
                      updateSigner(employees, employee, res, (err, res) =>
                        res.send({
                          success: true,
                          message: "employee updated successfully"
                        })
  ...
  validate.owner(
    ...
    (err, pass, loginuser) => {
      if (err) {
        ...
        pass ||
        (loginuser && c.predicate.isAdministrator(loginuser))
      ) {
        Employee.findById(req.params.id, null, (err, employee) => {
          if (err) {
            res.send({ success: false, message: err });
      ...
      if (err) {
        ...
        Employee.findById(req.params.id, null, (err, employee) => {
          if (err) {
            ...
            const signedRecord = employee.records.find(
              r => r._id.toString() === req.body.recordID
            );
            if (signedRecord) {
              if (
                req.body.forced ||
                !c.predicate.hasSignerOfRecordSigned(signedRecord, loginuser)
              ) {
                if (!req.body.pass) {
                  const leaveType = employee.activatedLeaveTypes.find(
                    lt => lt.name === signedRecord.leaveType
                  );
                  leaveType.consumes = calculateLastConsumes(
                    signedRecord.totals,
                    leaveType.consumes
                  );
                }
                signedRecord.signings.push(
                  createSigning(loginuser, req.body.pass)
                );
                employee.save(err => {
                  if (err) {
                    res.send({ success: false, message: err });
                    ...
                    res.send({
                      success: true,
                      message: "record signing updated successfully"
                    });
                  }
                });
              } else {
                res.send({
                  success: false,
                  message: "already signed the record"
                });
              }
            } else {
...
function createSigning({ _id, name, username, dept, level }, pass) {
  return {
    id: _id,
    name,
    username,
...
function calculateLastConsumes(recordTotals, leaveTypeConsumes) {
  const recordTotalsHalfHoursAmount =
    recordTotals.days * 16 + recordTotals.halfHours;
  const consumesHalfHoursAmount =
    leaveTypeConsumes.days * 16 +
    leaveTypeConsumes.halfHours -
    recordTotalsHalfHoursAmount;
  return {
...
function sendLeaveTakingEmail(taker, records) {
  collectSignersInfos(taker.signers, [], (err, signers) =>
    e.sendLeaveTakingEmail({ taker, signers, records })
  );
}

function updateRecord(employee, { activatedLeaveTypes, records }) {
  employee.activatedLeaveTypes = activatedLeaveTypes;
  employee.records = employee.records.concat(
    records.map(record => {
      return {
...
function collectSignersInfos(signers, signersInfos, cb) {
  ...
  if (signer) {
    Employee.findOne(
      c.conditions.employee(signer.username),
      (err, employee) => {
        if (err) {
          cb(err);
...
function updateSigner(
  employees,
  { username, employeeID, name, dept, level },
  res,
  cb
) {
...
function deleteSigner(employees, username, res, cb) {
  const employee = employees.pop();
  if (employee) {
    employee.signers = employee.signers.filter(
      signer => signer.username !== username
    );
    employee.save(err => {
      if (err) {
        res.send({ success: false, message: err });
...
function isLightweightUpdate(employee, { employeeID, name, level, dept }) {
  return (

      ...
      <v-layout>
        <v-flex xs12>
          <v-card class="elevation-12">
            <table-menu icon="person" :title="`${name} (${username})`" :enabled="{ export: true }">
                  ...
                  <td style="min-width:120px">
                    <v-tooltip bottom>
                      ...
                      <div v-if="fullControl || props.item.countdown">
                        <div>
                          {{localeConf.detail.dialog.consumes}} {{props.item.activatedDateType.consumes.days}} {{localeConf.detail.dialog.input.days}}
                          <span
                            v-if="props.item.activatedDateType.halfHoursEnabled"
                          >{{props.item.activatedDateType.consumes.halfHours / 2}} {{localeConf.detail.dialog.input.hours}}</span>
                        </div>
                        <div>
                          {{localeConf.detail.dialog.totals}} {{props.item.activatedDateType.totals.days}} {{localeConf.detail.dialog.input.days}}
                          <span
                            v-if="props.item.activatedDateType.halfHoursEnabled"
                          >{{props.item.activatedDateType.totals.halfHours / 2}} {{localeConf.detail.dialog.input.hours}}</span>
                        </div>
                        <div
                          v-if="props.item.activatedDateType.deadline"
                        >{{localeConf.detail.dialog.deadline}} {{props.item.activatedDateType.deadline}}</div>
                      </div>
                    </v-tooltip>
                  </td>
                    ...
                    <div v-for="signing in props.item.signings" :key="signing._id">
                      <v-tooltip bottom>
                        <v-chip
                          ...
                          text-color="white"
                          slot="activator"
                        >[{{signing.pass ? localeConf.recordlist.td.pass : localeConf.recordlist.td.reject }}] {{signing.name}}</v-chip>
                        <div>
